...
tf.flags.DEFINE_string("train_image_dir", "./data/train",
                       "Training image directory.")
tf.flags.DEFINE_string("test_image_dir", "./data/test",
...
def main(unused_argv):
    ...
    image_to_label = np.load('./data/image_to_label.npy')
    # Load image metadata from caption files.
    dog_train_dataset = []
    dog_test_dataset = []
    label_set=set()
    for item in image_to_label:
        label_set.add(item[1])
    dict_for_label={}
    for i,label in enumerate(label_set):
        dict_for_label[label]=i

    for item in image_to_label:
        label_set.add(item[1])
        dog_train_dataset.append(ImageMetadata(dict_for_label[item[1]],os.path.join(FLAGS.train_image_dir,item[0]+'.jpg')))

    list = os.listdir(FLAGS.test_image_dir)  # 列出文件夹下所有的目录与文件
    for i in range(0, len(list)):
        path = os.path.join(FLAGS.test_image_dir, list[i])
        dog_test_dataset.append(ImageMetadata(-1,path))

    train_cutoff = int(0.8 * len(dog_train_dataset))
    train_dataset =dog_train_dataset[0:train_cutoff]
    val_dataset=dog_train_dataset[train_cutoff:]
    test_dataset = dog_test_dataset
    ...
    np.save('label_set.npy',label_set)
